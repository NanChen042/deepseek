...
(function (ModelType) {
    ...
    ModelType["Reasoner"] = "deepseek-reasoner"; // DeepSeek-R1
})(ModelType || (ModelType = {}));
// 默认配置
...
class AIChatService {
    ...
    async chat(messages) {
        try {
            ...
            const response = await axios.post(`${API_CONFIG.baseURL}/chat/completions`, requestBody, {
                headers: {
    ...
    // 流式推理
    async streamReason(prompt, onChunk) {
        const previousConfig = this.config.model;
        try {
            this.config.model = ModelType.Reasoner;
...
// 为了保持向后兼容
export const chatWithDeepSeek = (messages) => {
...
export const sendChatMessage = async (content, model) => {
    try {
